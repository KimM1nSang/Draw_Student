...
public class SaveManager : MonoBehaviour
{
    ...
    public void LoadGameData()
    {
        ...
        if (File.Exists(fileStr))
        {
            ...
            for (int i = 0; i < saveData.rejectItem.Count; i++)
            {
                GameObject cardGO = Instantiate(GOTCHAManager.Instance.ResultCardPrefab, GOTCHAManager.Instance.ResultCardParentObject);
                cardGO.GetComponent<ResultCard>().SetUp(saveData.rejectItem[i]);
                GOTCHAManager.Instance.resultDict.Add(saveData.rejectItem[i], cardGO);

                GameObject GO = Instantiate(GOTCHAManager.Instance.ExeptItemPrefab, GOTCHAManager.Instance.ExeptParentObject);
                GO.GetComponent<ExeptItem>().SetUp(saveData.rejectItem[i], GOTCHAManager.Instance.CallReject);
                GOTCHAManager.Instance.rejectDict.Add(saveData.rejectItem[i], GO);
            }
